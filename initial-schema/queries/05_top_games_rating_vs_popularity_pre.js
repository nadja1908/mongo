// Pre-optimized: best rated vs most popular
...
// Create quick rank maps
const bayesRank = new Map();
byBayes.forEach((g,i)=> bayesRank.set(g.BGGId, i+1));
const ownedRank = new Map();
byOwned.forEach((g,i)=> ownedRank.set(g.BGGId, i+1));
...
// Hidden gems: high Bayes, low ownership -> top by Bayes where NumOwned in bottom 25%
...
// Hype games: high owned, lower Bayes
...
// Percent of users rating >=8 from ratings_distribution collection: compute bins 8.0-10.0 vs total
...
printjson({ label: 'top_games_rating_vs_popularity_pre', hiddenGems, hype, highBoth, pctHighRatings });
